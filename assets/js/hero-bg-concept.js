...
/* ── Product suite — tab switching + scroll activation ── */
    const productData = {
        askme: {
            pill: 'Lumenore Ask Me', icon: 'assets/images/products/Ask me Icon.svg', title: 'Conversational Analytics',
            desc: 'AI agents with context awareness and expertise that understand not just your question but also your business goal. Ask Me connects to your business data and, in response to your inquiries, brings external market intelligence to independently look into anomalies, identify underlying causes, and deliver governed insights that provide each team with the whole picture.',
            chips: ['Semantic Search', 'Contextual Intelligence', 'Natural Language Querying'],
            quote: '”Reporting cycle: 3 weeks → 4 days.”', quoteCite: 'Enterprise analytics team · faster self-serve reporting',
            cta: 'Explore Ask Me', ctaHref: 'lumenore-ask-me.html', animation: 'lottieAskMe', variant: '',
            visualImage: 'assets/images/products/askme-ui-preview.png'
        },
        insights: {
            pill: 'Lumenore Insights', icon: 'assets/images/products/Insight Icon.svg', title: 'Autonomous Analytics Engine',
            desc: 'An AI powered analytics engine that autonomously builds and updates dashboards, monitors KPIs, detects anomalies, and delivers critical insights to the right teams across channels, applications, and existing workflows.',
            chips: ['AI Dashboards', 'Data Story Telling', 'Real-Time Apps'],
            quote: '“80% reduction in ad-hoc IT data requests.”', quoteCite: 'Ford Credit · Financial Services · 5,000+ employees',
            cta: 'Explore Insights', ctaHref: '#', animation: 'lottieDashboard', variant: 'is-insights',
            visualImage: 'assets/images/products/insights-ui-preview.png'
        },
        datamagnet: {
            pill: 'Lumenore Data Magnet', icon: 'assets/images/products/Data Magnet icon.svg', title: 'Autonomous Data Operations for AI-Native Analytics',
            desc: 'An intelligent data foundation designed for AI-native analytics at scale that uses self-learning data pipelines, semantic context, and regulated data operations to automatically link, prepare, and orchestrate business data.',
            chips: ['100+ Connectors', 'Auto Pipeline Automation', 'Unified Data Stream'],
            quote: '“2M events/day, Zero engineers babysitting it.”', quoteCite: 'SiriusXM · Media and Entertainment · real-time streaming',
            cta: 'Explore Data Magnet', ctaHref: '#', animation: 'lottieDataMagnet', variant: 'is-datamagnet',
            visualImage: 'assets/images/products/datamagnet-ui-preview.png'
        }
    };
    const productOrder = ['askme', 'insights', 'datamagnet'];
    const productTabs  = document.querySelectorAll('.product-tab');
    const pdDetail     = document.getElementById('productDetail');
    const pdVisual     = document.querySelector('.product-visual');
    const pdHead       = document.querySelector('.products-head');
    const pdPill       = document.getElementById('pdPill');
    ...
    const pdQuote      = document.getElementById('pdQuote');
    const pdQuoteText  = document.getElementById('pdQuoteText');
    const pdQuoteCite  = document.getElementById('pdQuoteCite');
    const pdCta        = document.getElementById('pdCta');
    const pdLottieEl   = document.getElementById('pdLottie');
    const pdProgFill   = document.getElementById('pdProgFill');
    let currentProduct = 'askme';
    let lottieAnim = null;

    function loadLottie(varName) {
        const data = window[varName];
        if (!data) { setTimeout(() => loadLottie(varName), 100); return; }
        if (lottieAnim) { lottieAnim.destroy(); lottieAnim = null; }
        pdLottieEl.innerHTML = '';
        lottieAnim = lottie.loadAnimation({ container: pdLottieEl, renderer: 'svg', loop: true, autoplay: true, animationData: data, rendererSettings: { preserveAspectRatio: 'xMidYMid meet' } });
    }
    function renderVisual(d) {
        ...
        if (d.visualImage) {
            ...
            img.src = d.visualImage;
            img.alt = d.pill + ' preview';
            img.style.cssText = 'width:100%;height:auto;display:block;border-radius:16px;';
            pdLottieEl.appendChild(img);
        } else {
            loadLottie(d.animation);
        }
    }
    renderVisual(productData.askme);
    if (pdProgFill) pdProgFill.style.width = (1 / 3 * 100) + '%';

    function renderProduct(key) {
        const d = productData[key]; if (!d) return;
        currentProduct = key;
        if (pdProgFill) pdProgFill.style.width = ((productOrder.indexOf(key) + 1) / productOrder.length * 100) + '%';
        productTabs.forEach(t => { const on = t.dataset.product === key; t.classList.toggle('is-active', on); t.setAttribute('aria-selected', on ? 'true' : 'false'); });
        pdDetail.classList.remove('is-insights', 'is-datamagnet');
        pdVisual.classList.remove('is-insights', 'is-datamagnet');
        if (d.variant) { pdDetail.classList.add(d.variant); pdVisual.classList.add(d.variant); }
        pdPill.innerHTML = `<img class="pd-icon" src="${d.icon}" alt=""><span class="pd-sep" aria-hidden="true">&middot;</span>${d.pill}`;
        pdTitle.textContent = d.title; pdDesc.textContent = d.desc;
        pdChips.innerHTML = d.chips.map(c => `<span class="pd-chip">${c}</span>`).join('');
        if (d.quote) { pdQuoteText.textContent = d.quote; pdQuoteCite.textContent = d.quoteCite; pdQuote.hidden = false; } else { pdQuote.hidden = true; }
        pdCta.innerHTML = d.cta + ' <span class="pd-cta-arrow" aria-hidden="true">&rarr;</span>';
        if (d.ctaHref) pdCta.setAttribute('href', d.ctaHref);
        renderVisual(d);
    }

    let switchT = null;
    function activateProduct(key) {
        if (key === currentProduct || !productData[key]) return;
        pdDetail.classList.add('is-leaving'); pdVisual.classList.add('is-leaving'); pdHead.classList.add('is-leaving');
        clearTimeout(switchT);
        switchT = setTimeout(() => {
            renderProduct(key);
            pdDetail.classList.replace('is-leaving', 'is-entering');
            pdVisual.classList.replace('is-leaving', 'is-entering');
            pdHead.classList.replace('is-leaving', 'is-entering');
            requestAnimationFrame(() => requestAnimationFrame(() => {
                pdDetail.classList.remove('is-entering');
                pdVisual.classList.remove('is-entering');
                pdHead.classList.remove('is-entering');
            }));
        }, 90);
    }
    ...
    let cachedSectionTop = 0, cachedRange = 0, ticking = false;

    function cacheLayout() {
        cachedSectionTop = window.scrollY + productSection.getBoundingClientRect().top;
        cachedRange = productSection.offsetHeight - window.innerHeight;
    }
    cacheLayout();

    function updateByScroll() {
        if (cachedRange <= 0) return;
        let progress = Math.max(0, Math.min(0.999, (window.scrollY - cachedSectionTop) / cachedRange));
        const key = productOrder[Math.floor(progress * productOrder.length)];
        if (key !== currentProduct) activateProduct(key);
    }
    function onScroll() { if (ticking) return; ticking = true; requestAnimationFrame(() => { updateByScroll(); ticking = false; }); }

    function scrollToProduct(key) {
        const idx = productOrder.indexOf(key), seg = cachedRange / productOrder.length;
        window.scrollTo({ top: cachedSectionTop + seg * idx + seg * 0.5, behavior: 'smooth' });
    }

    let rotateTimer = null;
    function startAutoRotate() { if (reduceMotion) return; clearInterval(rotateTimer); rotateTimer = setInterval(() => { activateProduct(productOrder[(productOrder.indexOf(currentProduct) + 1) % productOrder.length]); }, 6000); }
    function stopAutoRotate() { clearInterval(rotateTimer); }

    function setupMode() {
        window.removeEventListener('scroll', onScroll); stopAutoRotate();
        if (deskQuery.matches) { window.addEventListener('scroll', onScroll, { passive: true }); updateByScroll(); } else { startAutoRotate(); }
    }
    productTabs.forEach(tab => tab.addEventListener('click', () => {
        if (deskQuery.matches) { scrollToProduct(tab.dataset.product); } else { activateProduct(tab.dataset.product); startAutoRotate(); }
    }));
    deskQuery.addEventListener('change', setupMode);
    window.addEventListener('resize', () => { cacheLayout(); if (deskQuery.matches) updateByScroll(); });
    setupMode();

    /* Product visual — mouse parallax */
    (function () {
        const visual = document.querySelector('.product-visual');
        if (!visual || reduceMotion) return;
        let rect = null, mTicking = false, pendingX = 0, pendingY = 0;
        const apply = () => { visual.style.setProperty('--mx', pendingX.toFixed(3)); visual.style.setProperty('--my', pendingY.toFixed(3)); mTicking = false; };
        visual.addEventListener('mouseenter', () => { rect = visual.getBoundingClientRect(); });
        ...
        visual.addEventListener('mouseleave', () => { pendingX = 0; pendingY = 0; if (!mTicking) { mTicking = true; requestAnimationFrame(apply); } rect = null; });
    })();

    /* ── Trusted logos — staggered flip animation ── */
    (function () {
        ...
        for (var i = 0; i < SLOTS; i++) {
            ...
            front.appendChild(makeImg(paired[i]));
            back.appendChild(makeImg(paired[i + SLOTS]));
        ...
        cards.forEach(function (c) { scheduleFlip(c, Math.random() * 6000); });
    })();
